function [h_fig,lims,ifg_data_RMSE,h_axes_all]=tps_plot(value_type,varargin)
...
addpath('/opt/stamps/StaMPS-4.1-beta/matlab/')
stdargin = nargin ; 
...
if aps_band_flag==1 
    ...
    if strcmp(small_baseline_flag,'y')         
        ...
        if aps_flag ==11
            eval(['K_tropo_powerlaw =  aps_bands.K_tropo_powerlaw_ifg_' num2str(ifg_number) ';']);            
            save(apsname,'K_tropo_powerlaw','-v7.3');           
            n_bands = size(K_tropo_powerlaw,2); 
        else
            eval(['ph_tropo_powerlaw =  aps_bands.ph_tropo_powerlaw_ifg_' num2str(ifg_number) ';']);
            save(apsname,'ph_tropo_powerlaw','-v7.3');
            n_bands = size(ph_tropo_powerlaw,2); 
        end
...
%             ph_all=ph_all.*conj(repmat(rc.ph_reref(:,ref_ifg),1,n_ifg));
    ...
    case {'u-dmos'}
      ...
      case {'u-ms'}
        ...
        scn=load(scnname);
        scla=load(sclaname);
        ph_all=uw.ph_uw - scn.ph_scn_slave - repmat(scla.C_ps_uw,1,size(uw.ph_uw,2)) ;
        clear uw scn scla
        ref_ps=ps_setref;
        fig_name = 'u-ms';
     %
   ...
   case {'a-o'}
	...
	case {'meanV-da'}
        ...
        aps=load(apsname);
        [aps_corr,fig_name_tca] = ps_plot_tca(aps,aps_flag);
        fig_name = ['meanV-da' fig_name_tca];
        meanV=meanV - aps_corr;
        lambda=getparm('lambda');
        ph_all=meanV*365.25/4/pi*lambda*1000;

        try
            save meanV meanV '-v7.3'
        catch
            save ~/meanV meanV '-v7.3'
            fprintf('Warning: Read access only, velocities saved in home directory\n')
        end
        clear aps aps_corr m
        textsize=0;
        units='mm/yr';
   ...
   case {'dtrnd'}
       disp('start to detrend')
       uw=load(phuwname);
       ph_uw=double(uw.ph_uw);
       [n_ps n_ifg] = size(ph_uw);
        ps_plot('v')
       [pol_lon, pol_lat] = getpts;
       pol = [pol_lon  pol_lat; pol_lon(1) pol_lat(1)];
       close
       in = inpolygon(lonlat(:,1),lonlat(:,2),pol(:,1),pol(:,2));
       lonlat_in = lonlat(in==1,:);
       ph_uw_in = ph_uw(inside==1,:);
       dtrnd_phuw = [];
        for i=1:n_ifg
       ...
       ph_uw = dtrnd_ph_uw-ph_unw_eni_osci;
       scla=load(sclaname);
   ...
   case {'v'}
        ...
        case {'v-gps'}
            ...
            clear scla
            fig_name = 'v-dGPS';
            GPS = load('/nas3/tohid/stamps_result/malazgirt/gps_malaz.dat');
            gpsRef = gps2los(lonlat,GPS);
        ...
        case {'v-eq'}
            ...
            clear scla
            ph_uw = rmEQs(ph_uw,lonlat,master_ix,day,n_ps); 
            fig_name = 'v-d_EarthquakeIsRemoved';
        case {'v-dg'}
            disp('remove tropospheric phase delay.')
            ztdPhas = load(ztdname);
            ztdPhas.ztd(isnan(ztdPhas.ztd)) = 0;
            ph_uw = ph_uw - double(ztdPhas.ztd) ;
            scla=load(sclaname);
            if isfield(scla,'C_ps_uw')
                ph_uw=ph_uw - scla.ph_scla - repmat(scla.C_ps_uw,1,size(ph_uw,2)); % master phase doesn't effect plot, but better for ts plot
            else
        ...
        case {'v-dms'}
            ...
            scn=load(scnname);
            scla=load(sclaname);
            ph_uw=uw.ph_uw - scn.ph_scn_slave - repmat(scla.C_ps_uw,1,size(uw.ph_uw,2)) - scla.ph_scla;
            clear uw scn scla
            fig_name = 'v-dms';
        ...
        if strcmp(value_type,'v-gps')
           lambda=getparm('lambda');
           gpsRefRad = gpsRef*4*pi/lambda/1000; 
           gpsRRD = (gpsRefRad/365.25*(day-master_day))';
           ph_uw=ph_uw-repmat((nanmean(ph_uw(ref_ps,:),1)-gpsRRD),n_ps,1);
           else
...
if ts_flag == 1
  ...
  % new TS plot button
  mButton=uicontrol('Style', 'pushbutton', 'Callback', 'clear ph_uw; tts_plot',...
...
save('prof','lonlat','ph_disp','-v7.3')
%%% dtrend velocity profile
pushbutton=uicontrol('Style', 'pushbutton', 'Callback', 'dtrnd',...
    'String','Detrend', 'Position', [20 125 60 20] ); % 
TextBox2=uicontrol('Style', 'edit','String','meanV', 'Position',...
      [20 100 60 20],'BackgroundColor',[1 1 1] );

%%% plot mean velocity profile
pushbutton=uicontrol('Style', 'pushbutton', 'Callback', 'tps_profile',...
    'String','Profile', 'Position', [20 50 50 20] ); % 
%%% plot displacement profile
pushbutton=uicontrol('Style', 'pushbutton', 'Callback', 'dpl_profile',...
    'String','Displacement', 'Position', [20 75 50 20] ); % 
%    
%TextBox=uicontrol('Style', 'edit','String','100', 'Position',...
%      [20 25 50 20],'BackgroundColor',[1 1 1] );
pushbutton=uicontrol('Style', 'pushbutton', 'Callback', 'tps_gacos',...
    'String','gacos', 'Position', [20 155 50 20] ); % 
